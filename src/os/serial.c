...
struct h8_3069f_sci {
    ...
    volatile uint8 rdr; // Reception Data Register
    volatile uint8 scmr;
};
...
#define H8_3069F_SCI_SCR_TE     (1<<5) // succeed enabled
#define H8_3069F_SCI_SCR_RIE    (1<<6) // receive interruption enabled
#define H8_3069F_SCI_SCR_TIE    (1<<7) // sending inerruption enabled
...
#define H8_3069F_SCI_SSR_RDRF   (1<<6) // finish receiving
#define H8_3069F_SCI_SSR_TDRE   (1<<7) // finish sending

// Start addresse of each SCI
static struct {
...
int serial_init(int index) {
    ...
    sci->scr = 0;
    sci->smr = 0;
    sci->brr = 64; // Generate 9600bps from 20MHz clock (set 80 when 25MHz)
    sci->scr = H8_3069F_SCI_SCR_RE | H8_3069F_SCI_SCR_TE; // can send and receive
    sci->ssr = 0;
